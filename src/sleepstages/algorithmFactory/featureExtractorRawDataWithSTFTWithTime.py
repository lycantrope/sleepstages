...
class FeatureExtractorRawDataWithSTFTWithTime(FeatureExtractor):
    def __init__(self):
        ...
        self.stft_nperseg = np.int64(np.floor(256.0 * self.stft_time_bin_in_seconds))
        ...
        self.binNum4spectrum = round(
            self.wholeBand.band_width / params.binWidth4freqHisto
        )
    ...
    def filtering(self, Zxx, freqs, lowerFreq, upperFreq):
        zipped = list(
            filter(lambda x: lowerFreq <= x[1] and x[1] < upperFreq, zip(Zxx, freqs))
        )
        return np.array([e[0] for e in zipped]), np.array([e[1] for e in zipped])

    def binning(self, Zxx, freqs, freqBinNum):
        binSize = np.int64(np.floor(1.0 * len(Zxx) / freqBinNum))
        Zxx_binned = np.array(
            [
                np.sum(np.abs(Zxx[(binID * binSize) : ((binID + 1) * binSize)]), axis=0)
                for binID in range(freqBinNum)
            ]
        )
        freqs_binned = np.array(
            [
                np.mean(freqs[(binID * binSize) : ((binID + 1) * binSize)], axis=0)
                for binID in range(freqBinNum)
            ]
        )
    ...
    def get_features(self, eegSegment, timeStampSegment, time_step):
        ...
        freqs, segment_times, Zxx = signal.stft(
            eegSegment, fs=self.samplingFreq, nperseg=self.stft_nperseg
        )
        ...
        # print('Zxx =', Zxx)
        Zxx_filtered, freqs_filtered = self.filtering(
            Zxx, freqs, self.wholeBand.bottom, self.wholeBand.top
        )
        # print('Zxx_filtered.shape =', Zxx_filtered.shape)
        Zxx_binned, freqs_binned = self.binning(
            Zxx_filtered, freqs_filtered, self.binNum4spectrum
        )
