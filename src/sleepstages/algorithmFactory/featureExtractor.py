...
class FeatureExtractor(ABC):
    ...
    def __str__(self):
        return f"FeatureExtractor(extractorType = {self.extractorType})"

    @abstractmethod
    def get_features(
        self, eegSegment: np.ndarray, timeStampSegment, time_step, local_mu, local_sigma
    ) -> np.ndarray:
        """A classmethod to get EEG features"""
    ...
    def featureExtraction(self, params, fileID):
        ...
        wsizeInTimePoints = (
            samplingFreq * windowSizeInSec
        )  # window size. data is sampled at 128 Hz, so 1280 sample points = 10 sec.
        binNum4spectrum = round(targetBand.band_width / binWidth4freqHisto)
        binArray4spectrum = np.linspace(
            targetBand.bottom, targetBand.top, binNum4spectrum + 1
        )

        # --------
        # load data
        # print('fileID = ' + str(fileID))
        filename = ".".join([eegFilePrefix, fileID, "pkl"])
        eeg_file_path = osp.join(eegDir, filename)
        with open(eeg_file_path, "rb") as dataFileHandler:
        ...
        while startSamplePoint + wsizeInTimePoints <= eeg.shape[0]:
            ...
            features = self.get_features(
                eegSegmentStandardized, timeStampSegment, time_step
            )
        ...
        featurePath = osp.join(featureDir, filename)
        print("extracting features for featurePath:", featurePath)
        with open(featurePath, "wb") as featureFileHandler:
